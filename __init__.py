...
class i2clcd():
    def __init__(self, i2c_bus=1, i2c_addr=0x27, lcd_width=20):
        """
...
def print_long_text(self, text):
    ...
    if isinstance(text, str):
        text = text.encode('ascii')
    # for i in range lo que hace es comenzar en 0 hasta len(text) pero de lcd_width en lcd_with
    #es decir que lo que tendremos son saltos, es decir, que si lcd_width son 20 
    #tendremos saltos de 20 en 20, ejemplo de valores que cogeria i seria 0,20,40 
    #teniendo en cuentra esto , line es una lista separada del numero de filas que tengamos.
    lines = [text[i:i + self._lcd_width] for i in range(0, len(text), self._lcd_width)]
    
    #Aqui tendremos que line_num sera el numero de filas, teniendo en cuenta que
    #esta la sabremos gracias al numero de iteraciones que ha hecho la i
    for line_num, line in enumerate(lines):
        #aqui vemos si line_num es menor que el numero de filas del lcd
        #como tenemos que len(LCD_LINES)=4 y line_num comienza en 0, la fila 4
        #para nosotros sera 3
        if line_num < len(LCD_LINES):  
            #printeamos por filas los caracteres
